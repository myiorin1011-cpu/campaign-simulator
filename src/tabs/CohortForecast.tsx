...
export function CohortForecast() {
  ...
  const rows = useMemo(() => {
    ...
    return Array.from({ length: cp.months }, (_, i) => {
      ...
      // ② 登録特典原価 = 新規集客数 × 3000pt × 0.22円 × 消化率0.7
      ...
      if (cp.campaignEnabled) {
        ...
        if (factor > 0) {
          ...
          if (cp.campaignApplyBonus ?? true) campaignBonus = installs * (cp.registrationBonusPt ?? 3000) * (cp.registrationBonusConsume ?? 0.7) * k
    ...
    <div className="space-y-6">
      ...
      <section className="card">
        <div className="flex items-center justify-between gap-3 flex-wrap">
          <div className="flex items-center gap-x-3 gap-y-1.5 flex-wrap text-sm" style={{ color: 'var(--text-secondary)' }}>
            ...
            <span className="chip-summary">登録特典{(cp.registrationBonusPt ?? 3000).toLocaleString()}pt×{((cp.registrationBonusConsume ?? 0.7) * 100).toFixed(0)}%</span>
      ...
      <Modal open={performerModalOpen} onClose={() => setPerformerModalOpen(false)} title="パフォーマー報酬原価 パラメータ（親データ）">
        ...
        <div className="text-[11px] mt-3 leading-relaxed" style={{ color: 'var(--text-muted)' }}>
          ...
          ② 登録特典原価 ＝ 新規集客数 × {cp.registrationBonusPt ?? 3000}pt × {cp.bonusPtCost ?? 0.22}円 × 消化率{((cp.registrationBonusConsume ?? 0.7) * 100).toFixed(0)}%<br />
